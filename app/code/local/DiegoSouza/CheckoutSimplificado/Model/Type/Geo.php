...
class DiegoSouza_CheckoutSimplificado_Model_Type_Geo
{
    ...
    public function usePayment($method_code = null)
    {
        ...
        else
        {
            ...
            {
                $method_code = Mage::getStoreConfig('checkoutsimplificado/general/payment_method');
                foreach ($payments as $payment)
                {
                    if ($method_code !== $payment->getCode())
                        continue;
    ...
    public function useShipping($method_code = null)
    {
        ...
        else
        {
            ...
            {
                $method_code = Mage::getStoreConfig('checkoutsimplificado/general/shipping_method');
                foreach ($rates['items'] as $rate)
                {
                    if ($method_code === $rate['code'])
                    {
    ...
    public function validateAddress($address)
    {
        ...
        $address->implodeStreetAddress();
        $a_form = Mage::getStoreConfig('checkoutsimplificado/address_form');
